...
const translations = {
  en: {
    ...
    inventoryChartAria: 'Inventory level and inventory position over time',
    inventoryChartAriaExpanded: 'Expanded inventory level and inventory position over time',
    inventoryChartDescription: ({
      ...
      `Inventory trend chart covering ${count} recorded ${Number(count) === 1 ? 'day' : 'days'}. Inventory level starts at ${startInventory} units and ends at ${endInventory} units. Inventory position starts at ${startPosition} units and ends at ${endPosition} units.`,
    ...
    inventoryExpandedChartTitle: 'Inventory Level & Inventory Position',
  ...
  'zh-Hant': {
    ...
    inventoryChartAria: '庫存水位與庫存部位隨時間變化',
    inventoryChartAriaExpanded: '放大的庫存水位與庫存部位圖表',
    inventoryChartDescription: ({
      count,
      endInventory,
      endPosition,
      ...
      `庫存趨勢圖涵蓋 ${count} 天記錄。庫存水位從 ${startInventory} 單位變化到 ${endInventory} 單位。庫存部位從 ${startPosition} 單位變化到 ${endPosition} 單位。`,
    ...
    inventoryExpandedChartTitle: '庫存水位與庫存部位',
  ...
  'zh-Hans': {
    ...
    inventoryChartAria: '库存水平和库存位置随时间变化',
    inventoryChartAriaExpanded: '放大的库存水平和库存位置图表',
    inventoryChartDescription: ({
      count,
      endInventory,
      endPosition,
      ...
      `库存趋势图涵盖 ${count} 天记录。库存水平从 ${startInventory} 单位变化到 ${endInventory} 单位。库存位置从 ${startPosition} 单位变化到 ${endPosition} 单位。`,
    ...
    inventoryExpandedChartTitle: '库存水平与库存位置',
